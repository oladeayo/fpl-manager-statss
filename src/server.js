...
app.get('/api/analyze-manager/:managerId', async (req, res) => {
  try {
    ...
    // Add processing for last 5 gameweeks performance
    const last5GWPerformance = [];
    ...
    for (const playerId in playerStats) {
      ...
      // Process last 5 GWs
      const last5GWs = playerHistory
        .filter(h => h.round > currentGameweek - 5 && h.round <= currentGameweek)
        .sort((a, b) => b.round - a.round);

      last5GWPerformance.push({
        name: player.web_name,
        gamesPlayed: last5GWs.length,
        totalPoints: last5GWs.reduce((sum, gw) => sum + gw.total_points, 0)
      });

      // Process upcoming fixtures
      const fixtures = playerHistoryResponse.data.fixtures.slice(0, 5).map(fixture => ({
    ...
    // Add new data to analysis object
    const analysis = {
      // Previous analysis properties remain the same
      last5GWPerformance,
      upcomingFixtures
    };
...
// Add endpoint for player images
